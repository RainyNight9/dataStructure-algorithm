...
1. 散列是一种`存储技术`
2. 散列后的数据可以`快速插入取用`
3. 在散列表上`插入、删除、取用数据非常快`，但是`查找数据却效率低下`
4. 数组长度是预先设定的，可以随时增加，所有元素根据和该元素对应的键，保存数组特定位置
5. 即使使用高效的散列函数，仍然存在两个键值相同的情况，这种现象称为`碰撞`
6. 数组的长度应该是一个`质数`，所有的策略都基于`碰撞`
7. `开链法`：两个键相同保存位置一样。开辟第二个数组，也称第二个数组为链
8. `线性探测法`属于开放寻址散列，查找散列位置，如果当前位置没有，继续寻找下一个位置。
9. 存储数据较大较合适。`数组大小>=1.5*数据（开链法）`，`数组大小>=2*数据（线性探测法）`
